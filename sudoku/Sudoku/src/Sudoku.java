...
public class Sudoku {
    ...
    public static void main(String[] args) {
        ...
        frame.setTitle("</Suduko Game / BVCOE>");
    ...
    public static void newGame() {
        int k = 0;
        ArrayList<Integer> randomnumber = getRandomNum();						// line 169
        ...
        boolean filledGrid = search(grid);
        if (filledGrid) {
        	  int rann = ran.nextInt(level);						//copy 0 and original values to make puzzle
              int c = 0;
              for (int i = 0; i < 9; i++) {
                  for (int j = 0; j < 9; j++) {
                      temp[i][j] = 0;
              ...
              p.setarray(grid, temp);					//288 panal file
              p.setTextLable();	
        }
      						// line 297
    }
